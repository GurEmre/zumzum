...
# generateBackground ...........................................................................................................
    def generateBackground(self,Lx,Ly,cueRadius,visibleRaduis):
        ...
        for i in range(0,R):
            cv.circle(im,(int((Lypx/2)),int(3*Lxpx/4)),i,gauss(3*Lxpx/5-i),2)

        for i in range(0,R):
            cv.circle(im,(int((Lypx/2)),int(Lxpx/4)),i,gauss(3*Lxpx/5-i),2)
...
# getNAS1 .......................................................................................................................
    def getNAS1(self,weighted=False):
        ''' no discrimination of local and global cue '''
        return np.sum(self.NASfunction1(self.swarm))/(self.ROBN*255)
# getNAS2 .......................................................................................................................
    def getNAS2(self,weighted=False):
        ''' no discrimination of local and global cue '''
        return np.sum(self.NASfunction2(self.swarm))/(self.ROBN*255)
# getQRs .......................................................................................................................
    def getQRs(self):
        '''
...
# LBA ***.................................................................................................................
    def LBA(self):
        for i in range(self.ROBN):
            ...
            elif self.swarm[i].waitingCue and (self.swarm[i].groundSensorValue1>0 or self.swarm[i].groundSensorValue2>0) and np.any(self.flagsR[i]==1):
...
class ROBOT(SUPERVISOR):
    def __init__(self,SUPERVISOR,name):
        ...
        self.position=[0,0]
        self.position2B=[0,0]
        self.groundSensorValue1=0
        self.groundSensorValue2=0
        self.waitingTime=0
...
# groundSense ..................................................................................................................
    def groundSense(self):#eğer bu aşağıda 631 te TRUE yerine bir pozisyon koşulu koyarsan sana gerekli ayrımı (aynı anda groundsense1=255-temp diğeri 0 eğer diğer loc taysa graoundsense2 255-temp diğeri 0) (log almak için sadece yapıyoruz violation yok)
        temp=self.ground[int(round(self.position[1])),int(round(self.position[0]))] #ground RGB intensity variable. bu positiondan farklı dikkat et
        R=self.SUPERVISOR.cueRadius
        # if [((self.Ylen/2))-R<(self.position[0])<self.Ylen/2+R] and [int(3*self.Xlen/4)-R<(self.position[1])<(3*self.Xlen/4)+R] or self.position[0]<= self.Ylen-(self.SUPERVISOR.visibleRaduis+2) and self.position[0]>= (self.SUPERVISOR.visibleRaduis+2)\
        #     and self.position[1]>=(self.SUPERVISOR.visibleRaduis+2) and self.position[1]<=self.Xlen-(self.SUPERVISOR.visibleRaduis+2): #True for cancelling condition

        if dist([self.position[0]-self.Ylen/4,self.position[1]-self.Xlen/2])<=R:
            self.groundSensorValue1=255-temp[0]
            self.groundSensorValue2=0

        # elif [self.Ylen/2-R<self.position[0]<self.Ylen/2+R] and [self.Xlen/4-R<(self.position[1])<(3*self.Xlen/4)+R] or self.position[0]<= self.Ylen-(self.SUPERVISOR.visibleRaduis+2) and self.position[0]>= (self.SUPERVISOR.visibleRaduis+2)\
        #     and self.position[1]>=(self.SUPERVISOR.visibleRaduis+2) and self.position[1]<=self.Xlen-(self.SUPERVISOR.visibleRaduis+2): #True for cancelling condition
        elif dist([self.position[0]-3*self.Ylen/4,self.position[1]-self.Xlen/2])<=R:
            self.groundSensorValue2=255-temp[0]
            self.groundSensorValue1=0
...
# aggregate ....................................................................................................................
    def aggregate(self,forced=False):
        self.groundSense()
        if ( any(self.SUPERVISOR.flagsR[int(self.robotName)]) and self.groundSensorValue1>0 and not self.delayFlag ) or forced:

            self.waitingTime=self.SUPERVISOR.Wmax*((self.groundSensorValue1**2)/((self.groundSensorValue1**2) + 5000))
            self.delayFlag=True
            forced=False
        elif ( any(self.SUPERVISOR.flagsR[int(self.robotName)]) and self.groundSensorValue2>0 and not self.delayFlag ) or forced:

            self.waitingTime=self.SUPERVISOR.Wmax*((self.groundSensorValue2**2)/((self.groundSensorValue2**2) + 5000))
            self.delayFlag=True
            forced=False
