...
def saveData(caller=None):
    ...
    if localMinima:
        data2BsavedStr=["NASwG1","NASG1","NASwL1","NASL1","NASwG2","NASG2","NASwL2","NASL2","log","Qtable","rewards","eps","alpha"]
        data2Bsaved=[NASwG1,NASG1,NASwL1,NASL1,NASwG2,NASG2,NASwL2,NASL2,log,QtableMem,reward,eps,alpha]
    else:
        data2BsavedStr=["NAS1","NAS2","log","Qtable","rewards","eps","alpha"]
        data2Bsaved=[NAS1,NAS2,log,QtableMem,reward,eps,alpha]
    ...
    if caller=='iterationDone':
        with open(fileName+commentDividerChar+comment+'.sup', 'wb') as supSaver:
            try:
                ...
                del sup.pos_getter
                del sup.rot_getter
                del sup.NASfunction1
                del sup.NASfunction2
                if localMinima:
                    del sup.NASGfunction1
                    del sup.NASfunction1
                    del sup.NASGfunction2
                    del sup.NASfunction2
...
def LOG():
    ...
    global NASw,NAS1,NAS2,log,eps,alpha,reward # caviat: if localMinima is true, its matrices are not globalized here
    ...
    if sup.getTime()%samplingPeriodSmall==0 and sup.getTime()-t>1:
        ...
        if method=='RL':
            ...
            if localMinima:
                NASG1[it,sampled],NASL1[it,sampled]=sup.getNAS1()    
                NASG2[it,sampled],NASL2[it,sampled]=sup.getNAS2()                 
                NASwG1[it,sampled],NASwL1[it,sampled]=sup.getNAS1(weighted=True)  
                NASwG2[it,sampled],NASwL2[it,sampled]=sup.getNAS2(weighted=True)                  
            else:
                # NASw[it,sampled]=sup.getNAS(weighted=True)
                NAS1[it,sampled]=sup.getNAS1()
                NAS2[it,sampled]=sup.getNAS2()
...
if __name__ == "__main__" or True:
    ...
    NAS1=np.zeros((iteration,sampledDataNum))
    NASw1=np.zeros((iteration,sampledDataNum))
    NAS2=np.zeros((iteration,sampledDataNum))
    NASw2=np.zeros((iteration,sampledDataNum))
    if localMinima:
        NASG1=np.zeros((iteration,sampledDataNum))
        NASwG1=np.zeros((iteration,sampledDataNum))
        NASL1=np.zeros((iteration,sampledDataNum))
        NASwL1=np.zeros((iteration,sampledDataNum))
        NASG2=np.zeros((iteration,sampledDataNum))
        NASwG2=np.zeros((iteration,sampledDataNum))
        NASL2=np.zeros((iteration,sampledDataNum))
        NASwL2=np.zeros((iteration,sampledDataNum))
